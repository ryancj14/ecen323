...
module riscv_forwarding_pipeline #(parameter INITIAL_PC = 32'h00400000) (
    ...
    // Added for forwarding functionality
    logic load_use_hazard;
    logic [4:0] ex_rs1;
    logic [4:0] ex_rs2;
    logic [31:0] data1;
    logic [31:0] data2_forward;
    logic [1:0] forwardA;
    logic [1:0] forwardB;
    //////////////////////////////////////////////////////////////////////
    // MEM: Memory Access
    ...
    always_ff@ (posedge clk) begin
        ...
        else if (load_use_hazard) if_PC <= if_PC;
        else if_PC <= if_PC + 4;
    end
    assign PC = if_PC;
    //////////////////////////////////////////////////////////////////////
    ...
    assign iMemRead = (mem_PCSrc | !load_use_hazard);
    //////////////////////////////////////////////////////////////////////
    // EX: Execute
    //////////////////////////////////////////////////////////////////////
    //ID to EX PC signal
    ...
    always_ff@ (posedge clk) begin
        ...
        else if (load_use_hazard | branch_mem_taken | branch_wb_taken) begin
            ex_MemWrite <= 0;
            ex_MemRead <= 0;
            ex_Branch <= 0;
            ex_RegWrite <= 0;
    ...
    assign forwardA = (load_use_hazard) ? 2'b00 :
                      (ex_rs1 == mem_rd) ? 2'b01 :
                      (ex_rs1 == wb_rd) ? 2'b10 : 2'b00;
    assign forwardB = (load_use_hazard) ? 2'b00 :
                      (ex_rs2 == mem_rd) ? 2'b01 :
                      (ex_rs2 == wb_rd) ? 2'b10 : 2'b00;
    
    assign data1 =  (forwardA == 2'b00) ? ex_data1 :
                    (forwardA == 2'b01) ? mem_aluResult :
                    (forwardA == 2'b10) ? wb_WriteData : 0;
    assign data2_forward = (forwardB == 2'b00) ? ex_data2 :
                            (forwardB == 2'b01) ? mem_aluResult :
                            (forwardB == 2'b10) ? wb_WriteData : 0;
    assign data2 = (ex_ALUSrc) ? ex_imm : data2_forward;
    assign ex_aluResult = (ex_ALUCtrl == AND) ? data1 & data2 :
                       (ex_ALUCtrl == OR)  ? data1 | data2 :
                       (ex_ALUCtrl == ADD) ? data1 + data2 :
                       (ex_ALUCtrl == SUB) ? data1 - data2 :
                       (ex_ALUCtrl == SLT) ? ($signed(data1) < $signed(data2)) :
                       (ex_ALUCtrl == XOR) ? data1 ^ data2 : 0;
    assign ex_Zero = (ex_aluResult == 0) ? 1'b1 : 1'b0;
    assign ex_branch_target = ex_PC + ex_branch_offset;
    assign ALUResult = ex_aluResult;
    
    // ID to EX rs1 and rs2 signals
    always_ff@ (posedge clk) begin
        if (rst) begin
            ex_rs1 <= 0;
            ex_rs2 <= 0;
    ...
    // load_use_hazard logic
    assign load_use_hazard = ((!branch_mem_taken && ex_MemWrite) 
    ...
    always_ff@ (posedge clk) begin
        ...
        else if (mem_rd == ex_rs2 && mem_MemtoReg) mem_write_data <= mem_read_data;
        else if (mem_rd == ex_rs2 && !mem_MemtoReg) mem_write_data <= mem_aluResult;
    ...
    //MEM to WB wb_rd signal
    always_ff@ (posedge clk) begin
        if (rst) wb_rd <= 0;
        else wb_rd <= mem_rd;
    end
    
    // branch_mem_taken to branch_wb_taken
    always_ff@ (posedge clk) begin
        if (rst) branch_wb_taken <= 0;
        else branch_wb_taken <= branch_mem_taken;
    end
